...
{
    FASSERT( nullptr == m_eventsSource,
        "we have ONLY TESTED this with one setter call per object" );
    // We can assume almost NOTHING about the LIFETIME of m_eventsSource.  We do
    // NOT own this pointer, and the QObject could be destroyed "at any time."
    // What we DO assume is that if 'eventFilter' is called then the event
    ...
    for( const auto type : WATCHED_TYPES )
    {
        ...
        if( event->type() == type )
        {
            ...
            logEventsNoMoreThanOncePerN.Do( [index]() {
                qInfo()
                    << "this GUI thread is running unblocked. Just got event from our index:"
                    << index;
            } );
        }
    }
